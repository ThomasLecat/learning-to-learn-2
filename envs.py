...
def create_env(env_id, client_id, remotes, num_trials, **kwargs):
...
def create_bandit_env(env_id, client_id, remotes, num_trials):
    env = gym.make(env_id)
    env = Vectorize(env)
    env = DiagnosticsInfo(env, isBanditEnv = True, num_trials = num_trials)
    env = Unvectorize(env)
    return env
...
def create_flash_env(env_id, client_id, remotes, **_):
    ...
    if env_id == 'flashgames.NeonRace-v0':
        # (original comment)  Better key space for this game.
...
class DiagnosticsInfoI(vectorized.Filter):
    def __init__(self, log_interval=503, **kwargs):
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if 'isBanditEnv' in self._kwargs and self._kwargs['isBanditEnv'] == True:
            terminating = self._episode_length % self._kwargs['num_trials'] == 0
        else:
            terminating = done
...
class CropScreen(vectorized.ObservationWrapper):
    """ (original comment)  Crops out a [height]x[width] area starting from (top,left) """
